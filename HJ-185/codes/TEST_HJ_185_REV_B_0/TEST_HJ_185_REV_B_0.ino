...
#define BLE_APP_CONFIG    	10		//BLE PIN14 - HIGH-Enable, LOW-Disable

/*BLUETOOTH*********************************************/
#include <SoftwareSerial.h>
SoftwareSerial bleSerial(BLE_TX, BLE_RX); //(RxTx) of MCU
//SoftwareSerial bleSerial(BLE_RX, BLE_TX); //(RxTx) of MCU
...
void setup(){
        ...
        bleSerial.begin(57600); 		//default 115200,..38400-sugested for 8MHz!!!
/*define I/O*/
        pinMode(LED, OUTPUT);
...
/*do some stuff here*/
    	...
    	//digitalWrite(BLE_SER_RX_ENABLE, LOW);	//enabless RX on BLE module
		ble_app_config_enable();//enables ble config from app
		//digitalWrite(BLE_APP_CONFIG, HIGH);		//enables ble config from app
}
