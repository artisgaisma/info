...
//ISP Programmer - USB Tiny

/*
**********************************************************************
BLE HJ-185-MH channels:
FFF0 - services
App 	<- 	BLE[FFF1] <-	RX port
App 	-> 	BLE[FFF2]  ->	TX port
App 	<->	BLE[FFF3]	for internal commands 
"Serial Bluetooth Terminal" app settings:
	Custom settings
	...
	3-Write charasteristics:
		...
		#TEMPLATE - APP <-> BLE <-> Transparant
			...
			FFF1
			FFF2
		#reconect BLE afters setting change!!!!!!!!!!!		
****************************-BLE MODULE INFO-******************************************
VCC - 1.7-3.6V
PIN14/P0.16 Config mode. Enable - HIGH, Disable - LOW
PIN9/P0.02 - Serial RX Enable. Enable - LOW, Disable - HIGH
<RD_NAME> <ST_NAME=xxx>
...
/*do some stuff here*/
	...
	sample_slider();	//prints group and param to FFT app once
}
...
/*OTHER_Functions*********************************************/
void forward() {  //from NRF - works
  if (bleSerial && Serial) {
    int byte;
    while ((byte = bleSerial.read()) > 0) Serial.write((char)byte);//write writes bytes
    while ((byte = Serial.read()) > 0) bleSerial.write((char)byte);//"print" works the same???
  }
}
